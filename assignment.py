...
class ModelClass(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        """
        n = data.shape[0]
        pred = torch.zeros(n,1,requires_grad=True)
    ...
    def loss(self, pred, label):
        ...
        if self.args.model_type == "svm": 
            ...
            N = pred.size(dim = 0)
            label = torch.unsqueeze(label,1) 
            loss = -torch.sum(label*torch.log(pred) + (1-label)*(torch.log(1-pred)))/N
            assert(loss.requires_grad==True)
            #torch.tensor(loss, dtype=torch.int8)
            pass       
            ######Your code ends here##########
        elif  self.args.model_type == "ranking":
            ##### TODO: Your code starts here######  
...
def evaluate(loader, model):
    ...
    eval_score = 0
    pp = torch.empty((0,),dtype=torch.float,requires_grad=True)
    nn = torch.empty((0,),dtype=torch.float,requires_grad=True)
    for data in loader:
        with torch.no_grad():
            ...
            if model.args.model_type == "svm": 
                ...
                pred[pred>=0] = 1
                pred[pred<0] = -1
                eval_score = torch.sum(label==pred)/label.size(dim = 0)
                pass       
                ######Your code ends here##########
                ...
                label = data[1]
                pred = torch.reshape(pred,(pred.size()[0],))

                
                #label = torch.unsqueeze(label,0)
                pp = torch.cat((pp, pred[label==1]),0)
                nn = torch.cat((nn,pred[label==0]),0)
                # l = ((pred[label==0]).unsqueeze(1) - pred[label==1])
                # l[l<0] = 0
                # eval_score += torch.sum(l==0)
...
if __name__ == '__main__':
    ...
    args.model_type = 'svm'  # Can change the model type here
